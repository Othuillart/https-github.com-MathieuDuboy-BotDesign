...
const FB_PAGE_TOKEN = "EAADHURv0ZB9kBACrJZALnUuDc0ZAspZCZBUZBt6FZAurWuPOOhx9p6pMjRgeMMUnNNGZCdNFAbQqn5ZCZAKHZAohNq0Q8oL046Qa53KRVujyBjnTAN5DBtJ7F3EEZAMTyZACFeROBBEU9lf7oP4GeRLz7O7ecE7jYyHlPYEGyvQZB25mR64AZDZD"; // saisir ici vos informations (infos sur session XX)
if (!FB_PAGE_TOKEN) {
						...
						.then(() => {
							...
							firebase.database()
								...
								.set({
									...
									nom: nom,
									nb_agression: 0,
									dernier_message: Date.now(),
									timestamp : Date.now(),
									genre: genre,
									derniere_relance_satisfaction : false,
									date: new Date()
										.toISOString(),
									date_first_interaction : Date.now()
								});
								...
								.set({
									...
									dernier_message: Date.now(),
									derniere_relance_satisfaction : false,
									genre: genre,timestamp : Date.now(),
									date: new Date()
										.toISOString(),
									date_first_interaction : Date.now()
								});
...
var actions = {
  ...
  estampille_last_interaction(keyid, entities, context, sessionId) {
    // ici dire la base de données Firebase que la deniere intaraction date de maintenant
      var date = Date.now();
			return firebase.database().ref().child('accounts/' + keyid).update({date_last_interaction : date, dernier_message : date}).then(function(error) {
  ...
  estampille_retour_3_semaines(entities, context, sessionId) {
    // ici dire la base de données Firebase que la deniere intaraction Spéciale (carte fidélité, etc) date de maintenant
      ...
      return firebase.database().ref().child('accounts/' + keyid).child('date_last_speciale').set(date).then(function(error) {
				console.log("erreur from firebas 14");
				var otherDatabase = otherApp.database();
				otherDatabase.ref().child('accounts/' + keyid).child('date_last_speciale').set(date);
			});
  },
	estampille_pas_content(keyid, entities, context, sessionId) {
    // ici dire la base de données Firebase que la deniere intaraction date de maintenant
		var date = Date();
		return firebase.database().ref('note_satisfaction').push().set({
			id_account: keyid,
			date: date,
			...
			pas_content : true
				...
				console.log("erreur from firebas 13");
				var otherDatabase = otherApp.database();
				otherDatabase.ref().ref('note_satisfaction').push().set({
				id_account: keyid,
				date: date,
				date_reception : Date.now(),
				note: false,
...
function stockFirebaseData(key_user, message, keyid, entities) {
	...
	if(entities && entities.intent) {
		console.log("entities.intent detecté");
		var intentions = entities.intent;
		var value = intentions[0].value;
		reponse_du_bot.suggestion0 = value;
		if(intentions[1])  {
			var value1 = intentions[1].value;
			var taux = intentions[1].confidence;
			if(taux >= 0.50) reponse_du_bot.suggestion1 = value1;
		}
		if(intentions[2])  {
			var value2 = intentions[2].value;
			var taux = intentions[2].confidence;
			if(taux >= 0.50)reponse_du_bot.suggestion2 = value2;
		}
		if(entities.Dire_Bonjour) {
			var value = "Intention pour dire Bonjour";
			reponse_du_bot.suggestion0 = value;
		}else if(entities.Dire_merci) {
			var value = "Intention pour dire Merci";
			reponse_du_bot.suggestion0 = value;
		}
	}else if(entities.Dire_Bonjour) {
		var value = "Intention pour dire Bonjour";
		reponse_du_bot.suggestion0 = value;
	}else if(entities.Dire_merci) {
		var value = "Intention pour dire Merci";
		...
		var value = "Animal détecté";
		reponse_du_bot.suggestion0 = value;
	}else if(entities.insultes) {
		var value = "Insulte détectée";
		reponse_du_bot.suggestion0 = value;
	}else if(entities.la_capsule_qui_fuit) {
		var value = "Gélule Doriance qui fuit";
		reponse_du_bot.suggestion0 = value;
	}else if(entities.alerte_danger_stop) {
		var value = "Danger / Pharmaco";
		reponse_du_bot.suggestion0 = value;
	}else {
		var value = false;
		reponse_du_bot.suggestion0 = value;
...
function stockFirebaseData2(key_user, message, keyid, entities) {
	var otherDatabase = otherApp.database();
                ...
                "sharable" : false,
							"elements": [{
								...
								"buttons": [{
									...
									"url": "https://botdesign.net/Naturactive/form-medical.html",
  						...
  						"attachment": {
  							...
  							"payload": {
  								...
  								"elements": [{
	  								...
	  								"title": "FAQ Doriance Autobronzant Gardenia",
	  								"image_url": "https://mon-chatbot.com/naturactive/images/Doriance_Autobronzant_Gardenia.jpg",
	  								"subtitle": "Vous avez détecté une anomalie dans votre pot de Doriance Autobronzant ?",
	  								"buttons": [{
	  									"type": "web_url",
	  									"url": "https://www.naturactive.fr/actualite/faq-doriance-autobronzant-gardenia",
	  									"title": "💊 En savoir plus"
	  								},{
  										"type": "element_share"
  									}]
	  							}, {
  									"title": "Verrerie du diffuseur cassée",
  									"image_url": "https://mon-chatbot.com/naturactive/logo_share.png",
  									"subtitle": "Afficher la procédure en cas de verrerie du diffuseur cassée",
  									"buttons": [{
  										"type": "postback",
    					...
    					},
              {
  					...
  					actions.reset_context(entities, context, sessionId).then(function() {
  						actions.envoyer_message_text(sessionId, context, entities, "Je suis désolée !").then(function() {
  							actions.timer(entities, context, sessionId, temps_entre_chaque_message).then(function() {
  								actions.envoyer_message_quickreplies(sessionId, context, entities, "Afin de répondre au mieux à votre besoin, voici ce que je peux vous conseiller.", quick).then(function() {
  									actions.estampille_pas_content(sessions[sessionId].key, entities, context, sessionId);
  								})
  							})
  						})
  					})
	                  ...
	                  if(messenger_ext) {
	                    ...
	                    var message = {
							...
							if(entities.intent && entities.intent[0].confidence <= 0.20) {
								...
								if (entities.la_capsule_qui_fuit) {
			...
			if (entry.hasOwnProperty('changes')) {
				entry.changes.forEach(function(changes) {
					...
					if (changes.field == "feed" && changes.value.item == "comment" && changes.value.verb == "add") {
						...
						// decouper la phrase
						var str2 = changes.value.message;
						var str = str2.toLowerCase();
						var substrings = ['fissurée','fissuré','percée','percé','huileux','cassée','cassé','fuit','fuite','capsule','déverssée','déversée','déversé','ouverte', 'orangé', 'gras', 'tâchée','orange'];
								...
								if(date1 < date2) {
									...
									fbMessage(recipientId, msg3).then(function(){
										...
										wit.message(text, sessions[sessionId].context, 3).then(({
											...
											apprentissage = false;
											stockFirebaseData(sessions[sessionId].key, text, keyid, entities);
											stockFirebaseData2(sessions[sessionId].key, text, keyid, entities);
										}).catch(console.error);
									}).catch(function(error) {
										console.log("Erreur envoi"+error);
									});
